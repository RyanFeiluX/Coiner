...
DependenciesTitle=外部依赖
DependenciesDescription=Coiner 所需的附加软件
DependenciesText=Coiner 需要 FFmpeg 和 ImageMagick 才能正常运行。%n%n1. FFmpeg（视频处理必需）%n   下载地址：https://www.gyan.dev/ffmpeg/builds/%n   或通过命令安装：winget install FFmpeg%n%n2. ImageMagick（字幕/文字渲染必需）%n   下载地址：https://imagemagick.org/archive/binaries/%n   或通过命令安装：winget install ImageMagick%n%n在 Windows 上安装 ImageMagick 后，%n请在 config.toml 中将 imagemagick_path 设置为 magick.exe 的路径。%n%n注意：内置的 imageio-ffmpeg 可以在找不到 FFmpeg 时自动下载，%n您可以跳过 FFmpeg，让其自动下载。
