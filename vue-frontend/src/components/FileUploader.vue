...
const handleFileRemove = (_file: any, fileList: FileItem[]) => {
  emit('update:modelValue', fileList);
  emit('remove', _file);
};
...
const handleUploadError = (error: any, _file: any, _fileList: FileItem[]) => {
  loading.value = false;
  emit('upload-error', error, _file, _fileList);
};
