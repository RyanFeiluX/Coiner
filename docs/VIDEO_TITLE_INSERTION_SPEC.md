...
4. **Hybrid Encoding (Title Enabled)**: When a title is enabled, the encoding splits into two stages:
   - **Stage 1 (FFmpeg)**: Silence prefix, pillarbox, subtitle burn-in, BGM mixing, and EBU R128 loudness normalization (`loudnorm=I=-16:TP=-1.5:LRA=11`) are rendered into a temp file. Loudnorm uses two-pass mode (first measures overall loudness, second applies linear gain) to preserve silence segments. This is 4-6× faster than MoviePy compositing for these elements.
